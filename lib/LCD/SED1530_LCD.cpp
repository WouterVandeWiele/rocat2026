...
#define EXPERIMENTAL_GPIO_ESP32
...
// }
  ...
  {
      #if defined(EXPERIMENTAL_GPIO_ESP32)
        mask = BIT(DATA[0]) | BIT(DATA[1]) | BIT(DATA[2]) | BIT(DATA[3]) | BIT(DATA[4]) | BIT(DATA[5]) | BIT(DATA[6]) | BIT(DATA[7]);
        // gpio_output_set(0, 0, BIT(A0) | BIT(RW) | BIT(EN) | mask, 0);
        pinMode(this->A0, OUTPUT);
        // pinMode(this->RW, OUTPUT); // will be set on the IO expander in init_display
      ...
      #elif !defined(EXPERIMENTAL_GPIO_ESP32)
        pinMode(this->A0, OUTPUT);
        pinMode(this->RW, OUTPUT);
        pinMode(this->EN, OUTPUT);
...
void SED1530_LCD::writeData(uint8_t lcdData) {
  // ESP_LOGI(LOG_LCD_TAG, "lcd write data %2x", lcdData);
    ...
    // gpio_output_set(BIT(A0), BIT(RW), 0, 0);

    // Write all 8 data pins at once
    // gpio_output_set(
    //   set_mask | BIT(A0),
    //   clear_mask | BIT(RW),
    //   0,
...
void SED1530_LCD::clearDisplay(void) {
  clearScreen(GLCD_COLOR_CLEAR);
}
